...
Approved Outlay`, each Physical / Unit Cost / Financial, plus Major
Component / Sub Component / Activity Master on the left.

**There is no `Sub Total` line and no R/NR marker column.** Level and
nature come off each sub-block's own closing `Total of <sub-component>`
label: "Class VIII" is Elementary, "Class XII" is Secondary, a trailing
`- NR` is non-recurring, "Recurring Components" is recurring. The block
closes on `Total of ICT and Digital Initiatives`. UP prints its
Elementary NR total without the `- NR` suffix; it is still NR.

Sub-component numbers run 127/128/129/130 in every state that prints
them **except Tripura**, which uses an `S.No` column numbered 44-47.
Punjab and Tripura also replace Major/Sub Component with `Sl.No.` and
head the sides `Proposal (Fresh)` / `Recommendation (Fresh)`.

**Row segmentation must come from the Activity Master column's cell
boxes**, not from `table.rows` — MuPDF also reports the spanning rows
the rowspanned Component cells create, and a page opening mid-rowspan
gives its first data row no leaf box at all (Bihar p65's `128.0.4`,
worth 1040.82 lakh, vanished that way). Andhra Pradesh, Chhattisgarh,
Kerala and Rajasthan rule *every wrapped line* as its own table row, so
those pages have to be rebuilt from text lines and rejoined.

Three header traps that cost real money:

- the word **`Final`** (from `Final Approved Outlay`) matches a "Fin"
  prefix and, taken as a Financial column, silently deletes
  Chhattisgarh's entire approved side;
- the **`Remarks` header word is centred** in a very wide column and
  sits ~20 pt right of where remark text starts, so the boundary has to
  come from the grid's cell edges (the same centred-header trap as the
  2024-25 `remarks_x` bug below);
- **Andhra Pradesh, Goa and Haryana print the header once** at the top
  and then run thirty pages without repeating it.

Numbers wrap three ways and all three are real figures cut in half, not
second values: no-gap horizontal (`6.4000`+`0`), after the decimal
point (`24693.`+`9`) and *before* it (`10092`+`.5`).

**The anchor must tolerate OCR damage.** Tripura's scan renders the
line as "Total oflCT and Disital Initiatives", which made a state with
a printed ₹37.4 Cr ICT block look like a no-ask. And a keyword scan
over a textless file proves nothing at all — Himachal Pradesh has zero
characters in both copies (`_minutes.pdf` is the 72 DPI re-upload;
`_minutes_9001.pdf` is intact at 200 DPI and is the copy to render).

**Jharkhand has no 2023-24 minutes anywhere** — the document library
lists only a corrigendum, which is a 500-school pre-school list with no
ICT content. Nothing to fetch.

### A row printed OUTSIDE the block that the block total still counts
The most valuable find of the 2023-24 pass, and the reason to check one
extra subtraction on every state.

*Madhya Pradesh 2023-24* reconciled on **all four** of its ICT
sub-blocks while sitting 496.80 / 248.40 below its printed block total.
That reads exactly like a §9 source-internal inconsistency and is not
one. The missing row is `75.0.x Smart Classroom for KGBV Hostels`, 207
hostels at 2.40 falling to 1.20, printed on p55 under `Project -
Innovative Activities- (NR) - (State Specific) (Elementary)` — **24
pages before the ICT block** — and the ICT block total counts it.
Restored, the state closes to the paisa on both sides.

Same family as the NIPUN Elementary Head gap and the Rajasthan MLE line
(`docs/nipun-passes.md`, `docs/reconciliation.md`), recurring one
extraction later in a different year and a different schema.

**A chain that closes on its sub-totals has not proved the block total
is the sum of them.** That is one subtraction, it is the only thing
that finds this class, and the reconciliation gate does not do it for
you. When it shows a gap, search the whole document's text layer for
the gap figure itself, **both sides at once** — the pair is far more
distinctive than either alone:

```python
re.search(r"\b496\.8\b|\b248\.4\b", page.get_text())
```

which found Madhya Pradesh's row on the first try. Staged in
`ict_outside_2324.py`, a third route beside the script and the vision
module, self-verifying against the printed block total.
